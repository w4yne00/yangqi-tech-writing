...
- Foundation 11 当前根目录完整单元测试为 133 项通过、0 项失败；材料合同模板、登记夹具、产品 JSON、Skill 结构、Markdown 链接、版本和公开文本脱敏检查均通过。
- Foundation 11 新增 18 项材料合同与样本登记外部行为测试，覆盖有效基础登记、模板完整性、合同或样本元数据缺失、结构化陈述效力跃迁、授权状态与用途范围、评审状态、未脱敏样本、项目受限用途、禁止持久化信息、正式要求身份、合成证据标记以及错误深度/联审声明。
