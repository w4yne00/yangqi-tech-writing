...
- Foundation 11 当前根目录完整单元测试为 131 项通过、0 项失败；材料合同模板、登记夹具、产品 JSON、Skill 结构、Markdown 链接、版本和公开文本脱敏检查均通过。
- Foundation 11 新增 16 项材料合同与样本登记外部行为测试，覆盖有效深度证据组合、模板完整性、合同或样本元数据缺失、授权范围、未脱敏样本、项目受限用途、禁止持久化信息、正式要求身份、合成证据标记、错误深度/联审声明和前向模型执行状态。
- 分发模板固定为 `basic_support`，其中的占位合成样本标记为 `deterministic-synthetic-fixture`、`model_execution: false`，不会被计入正式要求或真实案例，也不表示任何具体材料已经获得深度支持、联审支持或前向验证。
